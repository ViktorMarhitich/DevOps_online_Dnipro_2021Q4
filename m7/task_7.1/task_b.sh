...
function ripc
{
    ...
    cat $logname | grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | sort | uniq -c | sort -gr | awk
    '{print "There is " "\033[33m" $1 "\033[0m" " requests from IP: " $2}' | head -n $2
}
...
function reqcount
{
    ...
    logname=$1 
    cat $logname | awk '{print "\033[36m requests makes on \033[0m " $4"]"}' | sort | uniq -c | sort -gr I head -n $2
}
...
function searchbot
{
    ...
    echo -e "\033[33m What search bots have accessed the site? (UA + IP)\033[0m" logname=$1 
